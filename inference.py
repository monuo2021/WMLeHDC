...
train_set = WM811K('./data/wm811k/labeled/train/', transform=train_transform, filter_type='gaussian', kernel_size=5,
                   use_cache=True)
...
x_test = np.array([x for x, _ in test_loader.dataset], dtype=int)
y_test = np.array([y for _, y in test_loader.dataset], dtype=int)
...
print(f"Dataset WM811K loaded time: {time.time() - start_time}")
...
torch.seed()
x_train = torch.from_numpy(train_HVs.reshape((x_train.shape[0], -1))).type(torch.float).to(
    device)  # # (num_samples, 10000)
